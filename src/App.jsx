...
export default function App() {
  ...
  const [transactions, setTransactions] = useState(() => DB.get("transactions"));
  ...
  const save = useCallback((key, val) => { DB.set(key, val); }, []);
  ...
  useEffect(() => { save("invoices", invoices); }, [invoices, save]);
  useEffect(() => { save("accounts", accounts); }, [accounts, save]);
  useEffect(() => { save("customers", customers); }, [customers, save]);
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input, select, textarea { font-family: ${T.font}; color: ${T.text}; background: ${T.bg}; border: 1px solid ${T.border}; border-radius: 8px; padding: 10px 14px; outline: none; }
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <header style={{ height: 64, borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 28px", background: T.surface }}>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
              <div style={{ padding: "6px 14px", background: `${T.green}15`, color: T.green, borderRadius: 20, fontSize: 13, fontWeight: 600 }}>● Live</div>
            </div>
          </header>
